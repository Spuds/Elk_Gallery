...
namespace Addons\Levertine\Source\Helper\Image;
...
class ImageGD
{
	...
	public function loadImageFromFile($file)
	{
		...
		[$this->width, $this->height, $type] = $imageinfo;
		...
		$funcs = [
			IMAGETYPE_GIF => ['func' => 'imagecreatefromgif', 'ext' => 'png'],
			IMAGETYPE_PNG => ['func' => 'imagecreatefrompng', 'ext' => 'png'],
			IMAGETYPE_JPEG => ['func' => 'imagecreatefromjpeg', 'ext' => 'jpg'],
		];

		if ($this->hasWebpSupport())
		{
			$funcs[IMAGETYPE_WEBP] = ['func' => 'imagecreatefromwebp', 'ext' => 'webp'];
		}

		if (isset($funcs[$type]['func'])
			&& function_exists($funcs[$type]['func'])
			&& !empty($this->width) && !empty($this->height))
		{
	...
	public function loadImageFromString($string)
	{
		...
		[$this->width, $this->height] = $this->getImageSize();

		return true;
	}

	public function getImageFunction($format)
	{
		switch ($format) {
			case 'png':
				$imgfunc = 'imagepng';
				$quality = $this->compression['png'];
				break;
			case 'webp':
				$imgfunc = 'imagewebp';
				$quality = $this->compression['webp'];
				break;
			case 'jpg':
			default:
				$imgfunc = 'imagejpeg';
				$quality = $this->compression['jpg'];
				break;
		}

		return [$imgfunc, $quality];
	}
	...
	{
		[$imgfunc, $quality] = $this->getImageFunction($format);
	...
	public function resizeImageToMax($max_dimension, $dest_file, $format = 'jpg')
	{
		...
		else
		{
			...
			$new_x = (int) round($this->width / $largest * $max_dimension);
			$new_y = (int) round($this->height / $largest * $max_dimension);
