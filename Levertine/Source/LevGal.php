 ...
 */
class LevGal extends AbstractController
{
	public function action_index()
	{
		// Pass through if we ever get here
		$this->Media();
	}
	...
	public function Media()
	{
		...
		if (!allowedTo('lgal_view'))
		{
			...
			$_GET['action'] = '';
			writeLog(true);
			Http::fatalError('cannot_lgal_view');
		}

		// OK, dispatch time. First, can we load the class in question?
		$action = 'Home';
		if (!empty($_GET['sa']))
		{
		...
		try
		{
			$class = '\Addons\Levertine\Source\Action\\' . $action;
			$handler = new $class();
