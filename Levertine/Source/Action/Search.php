...
use Addons\Levertine\Source\Model\Search as SearchModel;
 ...
 */
class Search extends LevGalAbstract
{
	public function actionIndex()
	{
		...
		Txt::load('Levertine/LevGal-Search');
		...
		{
			$loaded = MembersList::load($context['search_member'], false, 'minimal');
		...
		if (!empty($_POST['submit']))
		{
			...
			$context['search_text'] = Sanitiser::sanitiseTextFromPost('search');
			...
			[$context['search_member'], $context['search_member_display']] = (new Member())->getFromAutoSuggest('search_member');
			...
			$albums = isset($_POST['alb']) && is_array($_POST['alb']) ? $_POST['alb'] : [];
			$selected_albums = [];
			if (!empty($context['hierarchies']['site']))
			{
	...
	public function actionRefine()
	{
		...
		{
			Http::hardRedirect($scripturl . '?media/search/');
		}
	...
	protected function getSearchTypes()
	{
		...
		$enabled_types = [];
		$types = ['image', 'audio', 'video', 'document', 'archive', 'generic', 'external'];
	...
	protected function performSearch()
	{
		...
		[$search_id, $has_results] = $search->performSearch();
