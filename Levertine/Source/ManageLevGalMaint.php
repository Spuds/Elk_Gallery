 ...
 */
class ManageLevGalMaint extends AbstractController
{
	public function pre_dispatch()
	{
		Txt::load('Levertine/ManageLevGal-Maint');
	...
	public function action_index()
	{
		...
		theme()->getTemplates()->load('Levertine/ManageLevGal-Maint');
	...
	public function action_levgal_maint_findfix()
	{
		...
		call_integration_hook('integrate_lgal_maint_findfix', [&$steps]);
		...
		[$step_complete, $total_substeps, $other_data] = $maintModel->$method($substep);
	...
	private function levgal_maint_rebuildsearch_item()
	{
		...
		$itemList = new \Addons\Levertine\Source\Model\ItemList();
	...
	public function action_levgal_maint_rebuildthumbs()
	{
		...
		while ($row = $request->fetch_assoc())
		{
			...
			$meta = !empty($row['meta']) ? Util::unserialize($row['meta'], ['allowed_classes' => false]) : [];
			...
			{
				$externalModel = new External($meta);
				if ($thumbnail = $externalModel->getThumbnail())
				{
					$itemModel->deleteFiles(['preview', 'thumb']);
