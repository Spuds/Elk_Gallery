 ...
 */
class ManageLevGalCFields extends AbstractController
{
	public function pre_dispatch()
	{
		Txt::load('Levertine/ManageLevGal-CFields');
		theme()->getTemplates()->load('Levertine/ManageLevGal-CFields');
	...
	public function action_levgal_adminCFields_index()
	{
		...
		if (isset($_POST['saveorder']))
		{
			...
			if (!empty($_POST['field']) && is_array($_POST['field']) && count(array_intersect($_POST['field'], array_keys($context['custom_fields']))) === count($_POST['field']))
			{
				...
				{
					$cfModel->updateField($field_id, ['field_pos' => $field_pos++]);
				}
	...
	private function lgal_get_default_field()
	{
		return [
			...
			'field_config' => [
				'required' => false,
				'bbc' => false,
				'default_size' => ['columns' => 30, 'rows' => 4],
				'max_length' => 255,
				'min' => 0,
				'max' => 0,
				'validation' => 'nohtml',
	...
	public function action_levgal_adminCFields_modify()
	{
		...
		if (isset($_REQUEST['savefield']))
		{
			...
			$context['custom_field']['field_name'] = Sanitiser::sanitiseTextFromPost('field_name');
			if (empty($context['custom_field']['field_name']))
			{
				$context['errors'][] = $txt['levgal_cfields_empty_field'];
			}
			$context['custom_field']['description'] = Sanitiser::sanitiseBBCTextFromPost('description');
			$context['custom_field']['placement'] = isset($_POST['placement']) && in_array((int) $_POST['placement'], [0, 1, 2], true) ? (int) $_POST['placement'] : 0;
			...
			$context['custom_field']['default_val'] = Sanitiser::sanitiseTextFromPost('default_val');
			$context['custom_field']['field_config']['max_length'] = Sanitiser::sanitiseIntFromPost('max_length', 0, 30000);

			if (!empty($_POST['all_albums']))
			{
				$context['custom_field']['field_config']['all_albums'] = true;
				$context['custom_field']['field_config']['albums'] = [];
			}
			else
			{
				$albums = [];
			...
			if (isset($_POST['field_type']) && in_array($_POST['field_type'], $context['valid_field_types'], true))
			{
				...
				switch ($context['custom_field']['field_type'])
				{
					case 'integer':
					case 'float':
						$context['custom_field']['field_config']['min'] = Sanitiser::sanitiseIntFromPost('min');
						$context['custom_field']['field_config']['max'] = Sanitiser::sanitiseIntFromPost('max');
						...
						{
							$context['custom_field']['default_val'] = LevGalBootstrap::clamp($context['custom_field']['default_val'], $context['custom_field']['field_config']['min'], $context['custom_field']['field_config']['max']);
						}
						break;
					case 'text':
					...
					case 'multiselect':
						...
						{
							$items = $_POST['select_option'];
							$_POST['default_selectmulti'] = isset($_POST['default_selectmulti']) ? (array) $_POST['default_selectmulti'] : [];
							if (isset($_POST['default_selectmulti'][0]))
							{
								$_POST['default_selectmulti'] = [];
							}
							$field_options = [];
							$default = [];
							foreach ($items as $k => $v)
